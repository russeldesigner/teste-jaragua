...
export default function App() {
  ...
  const canAccess = (view) => {
    ...
    if (userProfile.allowedViews.includes('all')) {
      return true;
    }
  ...
  const handleLoginSubmit = (e) => {
    ...
    const username = loginCreds.username.trim().toLowerCase();
    const password = loginCreds.password.trim().toLowerCase();
    ...
    const memberMatched = (data.membros || []).find(
      (m) =>
        String(m.sistemaLogin || '').toLowerCase() === username &&
        String(m.sistemaSenha || '').toLowerCase() === password
    );
    ...
    const profile = profilesConfig.find(
      (p) => p.id === memberMatched.sistemaPerfilId
    );

    if (!profile) {
      return showModal(
        'Atenção',
        'Usuário sem perfil válido atribuído.',
        'info'
      );
    }
    ...
    if (profile.id === 'recepcao') {
      setCurrentView('checkin');
    } else if (profile.id === 'obreiro') {
      setCurrentView('frequencia_obreiros');
    } else {
      setCurrentView('dashboard');
    }
  };
  ...
  const handleApprove = async (item) => {
    try {
      ...
      showModal(
        'Oficializado!',
        'Cadastro aprovado e movido para membros.',
        'success'
      );
    } catch (err) {
      showModal('Erro', err.message);
  ...
  const handleConvertVisitor = async (visitante) => {
    try {
      ...
      await updateRecord('visitantes', visitante.id, {
        foiConvertido: true
      });

      showModal(
        'Ficha Criada!',
        `${visitante.nome} agora possui um rascunho na Secretaria.`,
        'success'
      );
    } catch (err) {
      showModal('Erro', err.message);
  ...
  const closeMeeting = async () => {
    ...
    try {
      await updateRecord('reunioes', reuniao.id, {
        aberta: false
      });
    } catch (err) {
      showModal('Erro', err.message);
  ...
  const titles = {
    ...
    checkin: 'Recepção e Integração',
    frequencia_obreiros: 'Frequência de Obreiros',
    secretaria_lista: 'Cadastro de Membros',
    secretaria_form: 'Ficha de Membro',
  ...
  if (!userProfile) {
    return (
      <Login
        ...
        connected={Boolean(user && firebaseReady)}
      />
    );
  }

  return (
    <div className="min-h-screen bg-stone-50 flex text-stone-900 font-sans relative overflow-x-hidden">
      ...
      <Sidebar
        ...
        aprovacoesCount={(data.aprovacoes || []).length}
        logout={() => {
          setUserProfile(null);
          setLoginCreds({
            username: '',
      ...
      <main className="flex-1 min-w-0 flex flex-col h-screen overflow-y-auto bg-[#faf8f5] relative">
        <Header
          title={titles[currentView] || 'Plataforma AD'}
          userProfile={userProfile}
          connected={Boolean(user && firebaseReady)}
        ...
        {(error || !firebaseReady) && (
          <div className="mx-4 md:mx-8 mt-4 p-4 rounded-2xl bg-orange-50 border border-orange-200 text-orange-800 text-sm font-bold">
            {error ||
              'Firebase ainda não configurado. O layout abre, mas salvar dados exige as variáveis de ambiente.'}
          </div>
        )}

        <div className="p-4 md:p-8 max-w-6xl mx-auto w-full pb-32 overflow-x-hidden">
          {currentView === 'dashboard' && <Dashboard data={data} />}

          {currentView === 'secretaria_lista' && (
            <MembersList
              ...
              onNew={() => {
                setMemberData(initialMemberState);
                setEditId(null);
                setCurrentView('secretaria_form');
              }}
              onEdit={(m) => {
                setMemberData({
                  ...initialMemberState,
          ...
          {currentView === 'secretaria_form' && (
            <MemberForm
              memberData={memberData}
              setMemberData={setMemberData}
              data={data}
              onSave={handleSaveMember}
              onBack={() => setCurrentView('secretaria_lista')}
              editId={editId}
            />
          )}
          ...
          {currentView === 'frequencia_obreiros' && (
            <WorkersAttendance
              ...
              onPresence={registerPresence}
              onRemovePresence={(id) =>
                removeRecord('presencas', id)
              }
            />
          )}

          {currentView === 'autocadastro' && (
            <MemberForm
              memberData={memberData}
              setMemberData={setMemberData}
              data={data}
