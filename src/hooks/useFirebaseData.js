...
const makeEmptyData = () => Object.fromEntries(baseCollections.map((name) => [name, []]));
...
export function useFirebaseData() {
  ...
  const saveRecord = useCallback(async (collectionName, payload, editId = null) => {
    ...
    if (editId) {
      await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', collectionName, editId), { ...clean, updatedAt: new Date().toISOString() });
      return editId;
    }
    const docRef = await addDoc(collection(db, 'artifacts', appId, 'public', 'data', collectionName), { ...clean, createdAt: new Date().toISOString() });
    return docRef.id;
  }, []);
  ...
  const updateRecord = useCallback(async (collectionName, id, payload) => {
    if (!firebaseReady) throw new Error('Firebase não configurado.');
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', collectionName, id), { ...payload, updatedAt: new Date().toISOString() });
  }, []);

  return useMemo(() => ({ user, data, loading, error, saveRecord, removeRecord, updateRecord, firebaseReady }), [user, data, loading, error, saveRecord, removeRecord, updateRecord]);
}
